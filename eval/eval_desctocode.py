...
def main(args):
    ...
    for diag_type, dataset in test_dataset.items():
        ...
        for item in tqdm(dataset, desc=f"{diag_type}: "):
            messages = [
                {"role": "user", "content": [
                    {"type": "text", "text": f'You are provided with a topological description of a {configs[diag_type]} diagram, you need to generate the mermaid code based on this summary.\nSummary:\n\n{item["Description"]}'}
                ]}
            ]
            ...
            i+=1
        df.to_csv(os.path.join(args.output_dir, f"{configs[diag_type]}_Desc2Code.csv"), index=False)
