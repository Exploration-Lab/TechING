...
def pick_attributes_and_methods(classNames):
    ...
    class_attributes = defaultdict(list)
    for className in classNames:
        numAttributes = random.randint(2, 2)
        attribute = random.sample(all_classes[className], numAttributes)
        for att in attribute:
            class_attributes[className].append(att)
    for className in classNames:
        att = class_attributes[className][0]
        ...
        att = class_attributes[className][1]
        visibility_type = random.choice(visibility)
        method_prefix = random.choice(method_prefixes)
        method_suffix = random.choice(method_suffixes)
        attribute_name = f"{visibility_type}{method_prefix}{att}{method_suffix}\n"
        final_attributes[className].append(attribute_name)
                # print(attribute_name)
...
def connect_classes(noEdges, classNames):
    ...
    for _ in range(noEdges):
        connection = random.sample(classNames, 2)
        arrow = random.choice(arrows)
        text = f"{connection[0]} {arrow} {connection[1]}"
...
def save_image_and_code(mermaid_code, counter):
    ...
    # File name to save as .mmd
    code_file_name = f"./MermaidCodeEasy/ClassDiagram{counter}.mmd"
    image_file_name = f"./MermaidImageEasy/ClassDiagram{counter}.png"
...
def generate_dataset_sample():
    noClasses, noConnections = class_and_edges(minClasses=2, maxClasses=2) 
...
df.to_json("./ClassDiagramDatasetEasy.json", orient="records", indent=4)