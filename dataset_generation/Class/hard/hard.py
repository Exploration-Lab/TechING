...
def connect_classes(noEdges, classNames):
    ...
    arrows = arrowsUp if random.choice([True, False]) else arrowsDown
    ...
    diagram_choice = random.choice(range(16))

    if diagram_choice == 0: # 0000
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[0]} {arrow} {root}"
        connections.append(text)
        text = f"{filtered_class_names[1]} {arrow} {root}"
        connections.append(text)
        text = f"{filtered_class_names[2]} {arrow} {root}"
        connections.append(text)
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[3]} {arrow} {filtered_class_names[0]}"
        connections.append(text)
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[4]} {arrow} {filtered_class_names[2]}"
        connections.append(text)

    if diagram_choice == 1: # 0001
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[0]} {arrow} {filtered_class_names[1]}"
        connections.append(text)
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[1]} {arrow} {root}"
        connections.append(text)
    ...
    if diagram_choice == 2: # 0010
        ...
        connections.append(text)
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[2]} {arrow} {filtered_class_names[1]}"
        connections.append(text)
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[3]} {arrow} {filtered_class_names[0]}"
        connections.append(text)
    ...
    if diagram_choice == 4: # 0100
        ...
        connections.append(text)
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[3]} {arrow} {filtered_class_names[1]}"
        connections.append(text)
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[4]} {arrow} {filtered_class_names[2]}"
        connections.append(text)

    if diagram_choice == 5: # 0101
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[1]} {arrow} {root}"
        connections.append(text)
        text = f"{filtered_class_names[2]} {arrow} {root}"
    ...
    if diagram_choice == 8: # 1000
        ...
        connections.append(text)
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[4]} {arrow} {filtered_class_names[1]}"
        connections.append(text)

    if diagram_choice == 9: # 1001
        arrow = random.choice(arrows)
        text = f"{filtered_class_names[1]} {arrow} {root}"
        connections.append(text)
        text = f"{filtered_class_names[2]} {arrow} {root}"
...
def save_image_and_code(mermaid_code, counter):
    ...
    # File name to save as .mmd
    code_file_name = f"./MermaidCodeHard/ClassDiagram{counter}.mmd"
    image_file_name = f"./MermaidImageHard/ClassDiagram{counter}.png"
...
def generate_dataset_sample():
    noClasses, noConnections = class_and_edges(minClasses=6, maxClasses=6)
...
df.to_json("./ClassDiagramDatasetHard.json", orient="records", indent=4)