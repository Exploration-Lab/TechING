...
system_containers = {"Frontend": {"title": "Frontend", "desc": "The frontend of the system", "arrow": "Communicates with"},
                        "Backend": {"title": "Backend", "desc": "The backend of the system", "arrow": "Uses"},
                        "API": {"title": "API", "desc": "The API of the system", "arrow": "Creates request to"},
                        "Server": {"title": "Server", "desc": "The server of the system", "arrow": "Communicates with"},
                        "Client": {"title": "Client", "desc": "The client of the system", "arrow": "Uses"},
                        "Mobile App": {"title": "Mobile App", "desc": "The mobile app of the system", "arrow": "Sends data to"},
                        "Web App": {"title": "Web App", "desc": "The web app of the system", "arrow": "Uses"},
                        "Desktop App": {"title": "Desktop App", "desc": "The desktop app of the system", "arrow": "Creates request to"},
                        "Cloud": {"title": "Cloud", "desc": "The cloud of the system", "arrow": "Communicates with"},
                        "Mainframe": {"title": "Mainframe", "desc": "The mainframe of the system", "arrow": "Uses"},
                        "Middleware": {"title": "Middleware", "desc": "The middleware of the system", "arrow": "Sends data to"},
                        "Network": {"title": "Network", "desc": "The network of the system", "arrow": "Creates request to"},
                        "Security": {"title": "Security", "desc": "The security of the system", "arrow": "Communicates with"},
                        "Storage": {"title": "Storage", "desc": "The storage of the system", "arrow": "Uses"},
                        "Cache": {"title": "Cache", "desc": "The cache of the system", "arrow": "Creates request to"},
                        "Queue": {"title": "Queue", "desc": "The queue of the system", "arrow": "Communicates with"},
                        "Queue Management": {"title": "Queue Management", "desc": "The queue management system of the system", "arrow": "Creates request to"},
                        "API Handling": {"title": "API Handling", "desc": "The API handling system of the system", "arrow": "Uses"},
                        "API": {"title": "API", "desc": "The API of the system", "arrow": "Sends data to"},
                        "API Management": {"title": "API Management", "desc": "The API management system of the system", "arrow": "Creates request to"},
                        "API Gateway": {"title": "API Gateway", "desc": "The API gateway system of the system", "arrow": "Communicates with"},
                        "Mainframe": {"title": "Mainframe", "desc": "The mainframe of the system", "arrow": "Uses"}
                       }
...
def pick_class_names(personNo, system1, system2, containerNo):
    ...
    containerNames = random.sample(list(system_containers.keys()), containerNo)
    # print(classNames)
    return personNames, system1Names, system2Names, containerNames

# Connect classes with edges
def connect_classes(personNames, system1Names, system2Names, containerNames, containerSystemNo):
    connectionPairs = []
    combinedList = personNames + system1Names + system2Names
    combinedsystemList = system1Names + system2Names
    if containerSystemNo == 1:
        for person in personNames:
            connectionPairs.append((person, random.choice(system1Names + containerNames)))
        
        
        for system in system1Names:
            filtered_list = [name for name in system1Names if name != system]
            connectionPairs.append((system, random.choice(filtered_list + system2Names + containerNames)))
        for container in containerNames:
            filtered_list = [name for name in containerNames if name != container]
            connectionPairs.append((container, random.choice(filtered_list + system1Names + system2Names)))
        for system in system2Names:
            connectionPairs.append((system, random.choice(containerNames + system1Names)))
    
    elif containerSystemNo == 2:
        for person in personNames:
        ...
        for system in system1Names:
            ...
            connectionPairs.append((system, random.choice(filtered_list + system2Names + containerNames)))
        for system in system2Names:
            connectionPairs.append((system, random.choice(containerNames)))
        for container in containerNames:
            filtered_list = [name for name in containerNames if name != container]
            connectionPairs.append((container, random.choice(filtered_list + system1Names + system2Names)))
        
    return connectionPairs, containerSystemNo
...
def write_mermaid_code(personNames, system1Names, system2Names, containerNames, connectionPairs, containerSystemNo): 
    # Mermaid syntax as a string
    boundaries = 0
    system_and_containers = systems | system_containers
...
""" 
    ...
    if containerSystemNo == 1:
        mermaid_code += f"""        Container_Boundary(c1, "Internet Banking") {{\n"""
        for i in range(len(containerNames)):
            mermaid_code += f"""                Container({system_containers[containerNames[i]]["title"].replace(" ", "")}, "{system_containers[containerNames[i]]["title"]}", "{system_containers[containerNames[i]]["desc"]}")\n"""
        mermaid_code += f"""                }}\n"""
    mermaid_code += f"""            }}\n"""
    ...
    for system in system1Names:
        if system in databases:
            ...
            counter += 1
    if containerSystemNo == 1:
        for containers in containerNames:
            mermaid_code += f"""    Rel({containers.replace(" ", "")}, {connectionPairs[counter][1].replace(" ", "")}, "{system_containers[containers]["arrow"]}")\n"""
            counter += 1
    ...
    if containerSystemNo == 2:
        ...
        for containers in containerNames:
            mermaid_code += f"""    Rel({containers.replace(" ", "")}, {connectionPairs[counter][1].replace(" ", "")}, "{system_containers[containers]["arrow"]}")\n"""
            counter += 1        
    mermaid_code += """ UpdateLayoutConfig($c4ShapeInRow="5", $c4BoundaryInRow="4")"""
...
def save_image_and_code(mermaid_code, counter):
    ...
    # File name to save as .mmd
    code_file_name = f"./MermaidCodeHard/C4Diagram{counter}.mmd"
    image_file_name = f"./MermaidImageHard/C4Diagram{counter}.png"
...
def generate_dataset_sample():
    containerSystemNo = random.choice([1,2])
    if containerSystemNo == 1:
        person= class_and_edges(minBlocks=1, maxBlocks=2) 
        system1= class_and_edges(minBlocks=1, maxBlocks=1) 
        system2= class_and_edges(minBlocks=2, maxBlocks=2) 
        containerNo = class_and_edges(minBlocks=1, maxBlocks=1)
    if containerSystemNo == 2:
        person= class_and_edges(minBlocks=1, maxBlocks=2) 
        system1= class_and_edges(minBlocks=2, maxBlocks=2) 
        system2= class_and_edges(minBlocks=1, maxBlocks=1) 
        containerNo = class_and_edges(minBlocks=1, maxBlocks=1)
    personNames, system1Names, system2Names, containerNames = pick_class_names(person, system1, system2, containerNo)
    connectionPairs, containerSystemNo = connect_classes(personNames, system1Names, system2Names, containerNames, containerSystemNo)
    mermaid_code, boundaries = write_mermaid_code(personNames, system1Names, system2Names, containerNames, connectionPairs, containerSystemNo)
...
df.to_json("./C4DiagramDatasetHard.json", orient="records", indent=4)