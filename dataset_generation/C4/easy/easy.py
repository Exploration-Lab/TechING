...
def pick_class_names(personNo, system1):
    personNames = random.sample(list(persons.keys()), personNo)
    db_or_system = random.randint(0,1)
    if db_or_system == 0:
        system1Names = random.sample(list(databases.keys()), system1)
    else:
        system1Names = random.sample(list(systems.keys()), system1)


    # print(classNames)
    return personNames, system1Names

# Connect classes with edges
def connect_classes(personNames, system1Names):
    connectionPairs = []
    combinedList = personNames + system1Names 
    combinedsystemList = system1Names
...
def write_mermaid_code(personNames, system1Names, connectionPairs): 
...
""" 
    ...
    # Connections
    counter = 0
    # total_connections = len(personNames) + len(system1Names)
    ...
    if up_or_down == 1:
        ...
        for system in system1Names:
            if system in databases:
                mermaid_code += f"""    Rel({system.replace(" ", "")}, {connectionPairs[counter][0].replace(" ", "")}, "{databases[system]["arrow"]}")\n"""
            else:
                mermaid_code += f"""    Rel({system.replace(" ", "")}, {connectionPairs[counter][0].replace(" ", "")}, "{systems[system]["arrow"]}")\n"""


    top_down_or_LR = random.choice([0,1])
    if top_down_or_LR == 1:
        mermaid_code += """ UpdateLayoutConfig($c4ShapeInRow="1", $c4BoundaryInRow="1")"""
    else:
        mermaid_code += """ UpdateLayoutConfig($c4ShapeInRow="2", $c4BoundaryInRow="1")"""
...
def save_image_and_code(mermaid_code, counter):
    ...
    # File name to save as .mmd
    code_file_name = f"./MermaidCodeEasy/C4Diagram{counter}.mmd"
    image_file_name = f"./MermaidImageEasy/C4Diagram{counter}.png"
...
def generate_dataset_sample():
    person= class_and_edges(minBlocks=1, maxBlocks=1) # CHANGE for Level0, Level 1 and Level 2
    sysem1= class_and_edges(minBlocks=1, maxBlocks=1) # CHANGE for Level0, Level 1 and Level 2
    personNames, system1Names = pick_class_names(person, sysem1)
    connectionPairs = connect_classes(personNames, system1Names)
    mermaid_code, boundaries = write_mermaid_code(personNames, system1Names, connectionPairs)
...
df.to_json("./C4DiagramDatasetEasy.json", orient="records", indent=4)