...
def save_image_and_code(mermaid_code, counter):
    ...
    code_file_name = f"./StateDiagramEasy/MermaidCode/StateDiagram{counter}.mmd"
    image_file_name = f"./StateDiagramEasy/MermaidImage/StateDiagram{counter}.png"
    
    if not os.path.exists('./StateDiagramEasy/MermaidCode/'):
        os.makedirs('./StateDiagramEasy/MermaidCode/')
    if not os.path.exists('./StateDiagramEasy//MermaidImage/'):
        os.makedirs('./StateDiagramEasy/MermaidImage/')
...
def generate_dataset_sample():
    noBlocks, noEdges = blocks_and_edges(minBlocks=2, maxBlocks=4) # CHANGE for Level0, Level 1 and Level 2
    blockNames = block_names(noBlocks)  # Block names for Level 0 Diagram
    position_matrix = matrix(4,4) # CHANGE 4x4 Matrix for Level 0 Diagram, 6x6 for Level 1, 8x8 for Level 2
...
for i in range(3):
    ...
    counter += 1
    print(counter)
df.to_json("StateDiagramDatasetEasy.json", orient="records", indent=4)
